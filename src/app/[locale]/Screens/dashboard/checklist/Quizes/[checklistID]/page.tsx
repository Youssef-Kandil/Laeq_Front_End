...
function Quizes() {
    ...
    const edit_delete_Auth: boolean =
    Array.isArray(data) &&
    data?.length > 0 &&
    info?.email === data[0]?.checklists?.owner;
    ...
    const Cards = filteredData.map((card:TemplateType,indx:number)=>{
        return  <Quiz_card
                    ...
                    icon={ <div  style={{display:"flex",alignItems:"center",gap:"20px"}}>
                       {edit_delete_Auth&&<MdModeEdit style={{color:"rgba(29, 144, 238, 0.75)"}}  onClick={()=>router.push(`/${current_lang}/Screens/dashboard/checklist/Quizes/${checklistID}/EditTemplate/${card.id}`)}/>} 
                        {edit_delete_Auth&&<RiDeleteBinLine style={{color:"red"}} onClick={()=>handelDeleteTemplate(card.id??-1)}/>}
