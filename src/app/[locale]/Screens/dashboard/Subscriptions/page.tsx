...
import { useAdminSubscripe } from "@/app/Hooks/useSubscripe";
...
function Subscriptions() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useAdminSubscripe(Number(targetId));
  ...
  const modifiedData =
    SubscripeData?.map((Subscripe: SubscripeType) => ({
      ...
      subscripe_at: Subscripe?.subscripe_at
        ...
        : "—",
       action:<BottonComponent title="invoice" onClick={()=>{router.push(`/${current_lang}/Screens/dashboard/Subscriptions/Invoice/${Subscripe?.id}`)}}/>
    })) || [];
